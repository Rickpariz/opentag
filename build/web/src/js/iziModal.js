...
(function($){
	...
	iziModal.prototype = {
		...
		init: function (element, options) {
			...
			if(options.attached === 'top'){
			    this.$element.addClass('isAttachedTop');
			}

			if(options.attached === 'bottom'){
            ...
            (function setPositioning(){
	        	...
	        	$(document.head).append(that.mediaQueries);


	            // Adjusting vertical positioning
	            if(options.top !== null){

	            	if( String(options.top).indexOf("%") != -1){
	            		that.$element.css('top', options.top);
	            	} else {
	            		that.$element.css('top', parseInt(options.top));
	            	}

	            } else if (options.bottom !== null){

	            	that.$element.css('top', 'auto');
	            	that.$element.css('bottom', options.bottom);
		...
		close: function (param) {
            ...
            if(this.state == STATES.OPENED || this.state == STATES.OPENING){
				...
				var attachedClass = '';
				if(this.options.attached == 'top' || this.$element.hasClass('isAttachedTop')){
					attachedClass = 'isAttachedTop';
				}
				if(this.options.attached == 'bottom' || this.$element.hasClass('isAttachedBottom')){
					attachedClass = 'isAttachedBottom';
				}
				...
				if (transitionOut !== '') {
	                ...
	                this.$element.attr('class', [
						...
						this.$element.hasClass('isAttached') ? "isAttached" : "",
						attachedClass,
		...
		recalculateLayout: function(){
            ...
            if(this.state == STATES.OPENED || this.state == STATES.OPENING){
	                ...
	                var outerHeight = contentHeight + this.headerHeight + this.options.top + this.options.bottom;
                	...
                	if (outerHeight > windowHeight) {
                		...
                		if((contentHeight + this.headerHeight) > windowHeight){
				    	...
				    	if( (this.options.top > 0 || this.options.bottom > 0) && (this.options.attached !== 'top' || this.options.attached !== 'bottom')){
					    	this.$element.removeClass('isAttachedBottom');
					    	this.$element.removeClass('isAttachedTop');
				    	}
	                }
